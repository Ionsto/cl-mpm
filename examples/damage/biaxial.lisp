...
(defun setup (&key
  ...
  (let* ((width   80d-3)
    ...
    (format t "Length scale ratio ~E~%" (/ local-length h))
    (setf
     *sim*
     (cl-mpm/setup::make-simple-sim
      ...
      (list
       ...
       ;; :refinement 0;multigrid-refines
       )))
    ...
    (let* (;; (init-stress (cl-mpm/damage::mohr-coloumb-coheasion-to-tensile 40d3 angle))
       ...
       *sim*
       (cl-mpm/setup:make-block-mps
        ...
        :kt-res-ratio kt
        :kc-res-ratio kc
        :psi 0d0;(cl-mpm/utils:deg-to-rad angle-r)
      ...
      (time
        ...
        *sim*
        ...
        :plotter (lambda (sim)
                   ...
                   (push (cl-mpm/damage::sim-stats-damage-residual *sim*) data-d)
                   (incf iters)
                   (vgplot:semilogy x r "Residual"
                                    x data-d "Damage"
                                    )
                   ;; (plot-domain)
                   )
  ...
  (let ((damage 0d0)
    ...
    (case (cl-mpm/dynamic-relaxation::cell-mesh-index c)
      (0  (or (> damage-ybar 2d0)
                                        (> damage 0.20d0)
           ;; (and
           ;;  (> (varef (cl-mpm/mesh::cell-centroid c) 1) 0.06d0)
           ;;  (< (varef (cl-mpm/mesh::cell-centroid c) 1) 0.12d0))
           ))
      (1  (> damage 0.5d0))
      (2  (> damage 0.9d0))
      ;; (2  (> damage 0.85d0))
...
(defun test ()
  ...
  (dolist (refine (list 2))
    ...
    (cl-mpm/output::add-mp-output *sim* :SCALAR "damage-inc-crit"
                                  (lambda (mp)
                                    (if (> (cl-mpm/particle::mp-damage mp) 0d0)
                                        ...
                                        0d0)))
    (cl-mpm/output:add-mp-output *sim* :SCALAR "ll-length"
                                 (lambda (mp)
                                   (length (cl-mpm/particle::mp-local-list mp))))
    ...
    ;;  :VECTOR
    ;;  "force-frct"
    ;;  #'cl-mpm/mesh::node-reaction-force)
...
(defun plot-nonlocal-inter ()
  (let* ((find-pos (cl-mpm/utils:vector-from-list (list 0.10d0 0.063d0 0d0)))
...
(defun plot-damage-domain ()
  (when *sim*
    (let* ((find-pos (cl-mpm/utils:vector-from-list (list 0.10d0 0.055d0 0d0)))
           (mp (cl-mpm/setup::find-mp *sim* find-pos)))
      (setf (cl-mpm/damage::sim-enable-length-localisation *sim*) nil)
...
;; (defun profile ()
;;   (let ((pos (cl-mpm/utils::vector-from-list (list 0.1d0 0d0 0d0)))
;;         (mp (aref (cl-mpm::sim-mps *sim*) 0))
;;         )
;;     (sb-sprof:with-profiling (:mode :alloc :loop t :report :graph)
;;       (dotimes (n 1000)
;;         ;; (cl-mpm/damage::calculate-damage *sim* 1d0)
;;         (cl-mpm/damage::iterate-over-neighour-mps
;;          (cl-mpm:sim-mesh *sim*)
;;          mp
;;          (cl-mpm/particle::mp-local-damage mp)
...
;;     ;;    ;;   pos
;;     ;;    ;;   0.01d0
;;     ;;    ;;   (lambda (mesh mp-other dist)
;;     ;;    ;;     )
;;     ;;    ;;   ;; (funcall  mesh mp-other (sqrt distance))
;;     ;;    ;;  )
...
;;       ;; (grab-new-vector)
;;       (time
;;        (cl-mpm/utils::bpdotimes (i 10000)
