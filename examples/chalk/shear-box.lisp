...
(defun save-disp-preamble (sim output-dir)
  (with-open-file (stream (merge-pathnames output-dir "disp.csv") :direction :output :if-exists :supersede)
    (format stream "disp,load,plastic,damage,energy,l-left,l-right~%")))
(declaim (notinline save-disp))
(defun save-disp (sim output-dir)
  (unless (uiop:file-exists-p (merge-pathnames output-dir "disp.csv"))
    (save-disp-preamble sim output-dir))
  (with-open-file (stream (merge-pathnames output-dir "disp.csv") :direction :output :if-exists :append)
    (let ((disp-av *displacement-increment*)
          (load-av (get-load))
          (p-av (cl-mpm/dynamic-relaxation::get-plastic sim))
          (d-av (cl-mpm/dynamic-relaxation::get-damage sim))
          (e-av 0d0)
          )
...
(defun plot-domain ()
  (let ((sim *sim*))
    ...
    (let* ((ms (cl-mpm/mesh:mesh-mesh-size (cl-mpm:sim-mesh sim)))
           ...
           (ms-y (second ms))
           (offset *box-size*
             ;; (* 2d0 (cl-mpm/mesh:mesh-resolution (cl-mpm:sim-mesh sim)))
                   ))
  ...
  (when (> (length *data-v*) 0)
    (let* ((width 0.06d0)
      ...
      (vgplot:plot
       ...
       "Damage"
       ;; (mapcar (lambda (x) (* x 1d3)) *data-disp*)
       ;; (mapcar (lambda (x) (* x 5d0)) *data-plastic*)
       ;; "Plastic"
       ;; (mapcar (lambda (x) (* x 1d3)) *data-disp*)
...
(defun setup-test-column (size offset block-size &optional (e-scale 1) (mp-scale 1)
  ...
  (let* ((multigrid-refinement 1)
    ...
    (progn
      ...
      ;; (setf (cl-mpm::sim-velocity-algorithm sim) :PIC);;Conv in 29
      (setf (cl-mpm::sim-velocity-algorithm sim) :QUASI-STATIC);; Conv in 29
...
(defun make-penalty-box (sim left-x right-x height friction-scale offset &key (epsilon-scale 1d2)
  ...
  ;;           *shear-box-struct-right*
  ;;           *shear-box-struct-right-static*))
  ;;   (cl-mpm/penalty::walk-penalty-list
  ;;    bcs
  ;;    (lambda (bc)
  ;;      (setf (cl-mpm/penalty::bc-penalty-stiffness-scale bc) 2d0))))
  )
...
(defmethod cl-mpm/dynamic-relaxation::convergence-criteria ((sim cl-mpm::mpm-sim))
  (let* ((inst-load (/ (get-piston-load *sim*) 0.06d0))
    ...
    (or (not *use-rigid-piston*)
        (< crit (cl-mpm/dynamic-relaxation::sim-convergence-critera sim) ))))


(defparameter *piston-disp-n* 0d0)
(defparameter *piston-disp* 0d0)

;; (defmethod cl-mpm::finalise-loadstep :after ((sim cl-mpm::mpm-sim))
;;   (when *use-rigid-piston*
;;     (setf *piston-disp-n* (cl-mpm/utils::varef (cl-mpm/penalty::bc-penalty-distance-center-point *piston-penalty*) 1))
...
(defun make-piston (box-size box-offset surcharge-load epsilon-scale piston-scale)
  ...
  (let* ((control-stiffness
           ...
           ;;    (/ 1d1 epsilon-scale))
           ;; (/ piston-scale epsilon-scale)
           (* (/ 1d0 (* piston-scale epsilon-scale *elastic-constant*
                        ;; 2d0
                        ;; 0.06d0
                        )))
           )
    ...
    ;; (setf (cl-mpm/penalty::bc-penalty-stiffness-scale *piston-penalty*) (* 1d0 piston-scale epsilon-scale))
    (setf (cl-mpm/penalty::bc-penalty-stiffness-scale *piston-penalty*) (* 1d0 piston-scale))
                ...
                do
                   (progn
                     ...
                     (let ((load-av 0d0)
                       ...
                       (if (> work 0d0)
                           (setf energy (abs (/ (cl-mpm/dynamic-relaxation::estimate-energy-norm *sim*) work)))
                           (setf energy 0d0))
...
(defun run-static (&optional (output-directory "./output/")
  ...
  (with-open-file (stream (merge-pathnames output-directory "disp.csv") :direction :output :if-exists :supersede)
    (format stream "disp,load,plastic,damage,energy,l-left,l-right~%"))
  (with-open-file (stream (merge-pathnames output-directory "conv.csv") :direction :output :if-exists :supersede)
  ...
  (let* ((dt (cl-mpm:sim-dt *sim*))
    ...
    (let ((total-iter 0)
      ...
      (time (loop for steps from 0 below load-steps
                  while *run-sim*
                  do
                     (let ((target-disp (if disp-function (funcall disp-function steps load-steps displacment)
                                            (* steps disp-inc))))
                       (incf *displacement-increment* (- target-disp current-disp))
                       (setf current-disp target-disp)
                         ...
                         (let ((substeps 50)
                            ...
                            *sim*
                            ...
                            (lambda (i energy oobf)
                              ...
                              (cl-mpm/output:save-vtk-nodes (merge-pathnames output-directory (format nil "sim_conv_nodes_~5,'0d_~5,'0d.vtk" steps (+ 1 i))) *sim*)
                              (setf *piston-confinement* 0d0
                                    *piston-steps* 0)

                              (let ((inelastic
                                      ;; t
                                      (and (< energy 1d-2) (< oobf 1d-2))
                                      ))
                                (setf (cl-mpm::sim-enable-damage *sim*) (and inelastic enable-damage))
                                ...
                                ;;      (setf (cl-mpm/particle::mp-enable-plasticity mp) (and inelastic enable-plasticity)))))
                                )
                              ;; (when (and elastic-step
                              ;;            (or
                              ;;             (and (< energy 1d-1) (< oobf 1d-1))
                              ;;             no-trial-elastic))
                              ;;   ;; (setf (cl-mpm:sim-damping-factor *sim*)
                              ;;   ;;       (* 0.01d0
                              ;;   ;;          (cl-mpm/setup::estimate-critical-damping *sim*)))
                              ...
                              (with-open-file (stream (merge-pathnames output-directory "conv.csv") :direction :output :if-exists :append)
                                (format stream "~D,~D,~f,~f,~f,~f~%" total-iter steps (get-plastic) (get-damage) 
                                        oobf energy))
                              )))
                         (setf load-av (get-load))
                         (setf disp-av *displacement-increment*)
...
;;                          (* (expt factor 2) dist)
;;                          (* -0.5 dist)))))
(defun multi-load-unload (p dist nreversal)
  (let ((points (list 0d0)))
    (dotimes (i nreversal)
      (push dist points)
      (push 0d0 points))
    (list-interp p points)))

(defun complex-load-unload (p dist nreversal)
  (let ((points (list 0d0 (* dist 0.25d0)
                      0d0 (* dist 0.5d0)
                      0d0 dist
                      0d0)))
...
(defun current-disp (loadstep total-disp sub-loadsteps)
  (* total-disp 2 (floor (- (/ loadstep sub-loadsteps)
                             ...
                             do
                                (dolist (damping '(1d0))
                                  (let (;(piston-scale 10d0)
                                    ...
                                    ;; (change-class *sim* 'cl-mpm/dynamic-relaxation::mpm-sim-octree-damage-quasi-static :refinement 3)
                                    ...
                                    ;;           ;; (> damage-ybar (cl-mpm/dynamic-relaxation::cell-mesh-index c))
                                    ;;           (case (cl-mpm/dynamic-relaxation::cell-mesh-index c)
                                    ;;             (0  (or (> damage-ybar 1d0)
                                    ;;                     (> damage 0d0)))
                                    ;;             (1  (or (> damage-ybar 1d0)
                                    ;;                     (> damage 0d0)))
                                    ;;             (2  (or (> damage-ybar 1d0)
                                    ;;                     (> damage 0d0)))
                                    ;;             (3  (or (> damage-ybar 1d0)
                                    ;;                     (> damage 0d0)))
                                    ;;             (t nil))
                                    ;;           )))
...
(defgeneric get-all (bc))
(defmethod get-all ((bc cl-mpm/bc::bc))
  (list bc))
(defmethod get-all ((bc cl-mpm/penalty::bc-penalty-structure))
  (loop for sub-bc in (cl-mpm/penalty::bc-penalty-structure-sub-bcs bc)
        append (get-all sub-bc)))

(defun disp-profile (step load-steps target-disp)
  (let ((current (/ step load-steps))
        (point 0.5d0)
        (disp-point 0.2d0)
        )
    (if (< current point)
        (* disp-point (/ current point))
        (+ disp-point
           (* (- target-disp disp-point) (/ (- current point) (- 1d0 point))))))
  )
...
(defun test-adaptive ()
  ...
  (let ((total-disp 6d-4)
     ...
     *sim*
     ...
     :post-iter-step (lambda (i o e)
                       (push disp-av *displacement-increment*)
                       (push load-av (get-load))
                       (setf d-av (cl-mpm/dynamic-relaxation::get-damage *sim*))
                       (format t "Penalty load ~E~%" (get-piston-load *sim*)))
     :post-conv-step
     (lambda (sim)
       (save-disp sim output-dir))
               ...
               (let ((piston-scale 1d0)
                 ...
                 (let ((total-disp 1d-3)
                    ...
                    *sim*
                    ...
                    :loading-function
                    (lambda (percent)
                      (setf *displacement-increment*
                            (complex-load-unload percent total-disp 1))))))))))


;; (let ((x (loop for x from 0 to 1d0 by 0.01d0 collect x)))
;;   (vgplot:plot
;;    x
