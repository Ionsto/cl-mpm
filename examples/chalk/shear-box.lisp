...
(defun setup-test-column (size offset block-size &optional (e-scale 1) (mp-scale 1)
  ...
  (let* ((multigrid-refinement 1)
         (sim (cl-mpm/setup:make-simple-sim
               ...
               ;; :args-list (list :refinement multigrid-refinement)
              ))
    ...
    (progn
      ...
      (setf (cl-mpm::sim-velocity-algorithm sim) :BLEND);; Conv in 29
       ...
       (lambda ()
         (let* ((inst-load (/ (get-piston-load *sim*) 0.06d0))
           ...
           (let* ((delta (- (- current-load target-load)))
                  (penalty-inc (* ;; (cl-mpm:sim-dt *sim*)
                                (cl-mpm::sim-dt-scale *sim*)
                                  (* delta control-stiffness))))
...
(defun run-static (&optional (output-directory "./output/")
  ...
  (vgplot:figure)
  (plot-load-disp))
(defun run-static-reversals (&optional (output-directory "./output/")
                     &key
                       (load-steps 50)
                       (dt-scale 0.5d0)
                       (displacment 0.5d-3)
                         ...
                         (let ((substeps 50)
                            ...
                            *sim*
                            ...
                            (lambda (i energy oobf)
                              ...
                              (let ((inelastic (and (< energy 1d-1) (< oobf 1d-1))))
                                (setf (cl-mpm::sim-enable-damage *sim*) (and inelastic enable-damage))
                                (cl-mpm:iterate-over-mps
                                 (cl-mpm:sim-mps *sim*)
                                 (lambda (mp)
                               ...
                               do
                                  (dolist (damping '(1d0))
                                    (let (;(piston-scale 10d0)
                                      ...
                                      ;;  :loading-function (lambda (percent)
                                      ;;                      (setf *displacement-increment* (* 6d-3 percent))))
                                      ;; (run-static
                                      ;;  (format nil "../ham-shear-box/output-~A_~f_~D_~f_~f_~f-~F/" name refine mps scale piston-scale epsilon-scale s)
                                      ;;  :displacment 6d-3
                                      ;;  ;; :surcharge-load s
                                      ;;  :load-steps 20
                                      ;;  :disp-function (lambda (step load-steps target-disp)
...
(defun test-resolve ()
  (let* ((mps (cl-mpm:sim-mps *sim*))
         ...
         (dt (cl-mpm:sim-dt *sim*))
         (mesh (cl-mpm:sim-mesh *sim*))
         (bcs (nth 3 (cl-mpm:sim-bcs-force-list *sim*)))
         (corner (cl-mpm/utils:vector-from-list (list 0d0 0d0 0d0)))
         )
    ;; (sb-profile:profile)
    ;; (sb-profile:reset)
    ;; (sb-profile:profile "CL-MPM")
    ...
    ;; (sb-profile:profile "CL-MPM/CONSTITUTIVE")
    ;; (sb-profile:profile "CL-MPM/PENALTY")
    (format t "MPs ~D~%" (length mps))
    (let* ((bc (aref bcs 3))
           ;; (sub-bcs (cl-mpm/penalty::bc-penalty-structure-sub-bcs bc))
           ;; (bc (nth 1 sub-bcs))
           )
      (format t "~A" (get-all bc))
      (time
       (dotimes (i 10)
         (progn (cl-mpm/bc:apply-bc bc nil mesh dt)))))

                                        ;loop for bc across bcs do
    ;; (let ((bc (aref bcs 0)))
                ;;   (cl-mpm/penalty::resolve-closest-contact bc corner))
...
;; (let* ((load-steps 100)
;;        (target-disp 1d0)
;;        (x (loop for x from 0 to load-steps collect x))
;;        (d (loop for i in x collect (disp-profile i load-steps target-disp)))
;;        )
;;   (vgplot:plot x d)
;;   (vgplot:xlabel "loadstep")
;;   (vgplot:xlabel "disp")
;;   )
