  ...
  (let* ((sim (cl-mpm/setup::make-block (/ 1 e-scale)
    ...
    (progn
      ...
      (let ((mass-scale 1d0))
        ...
        (setf (cl-mpm:sim-damping-factor sim) (* density 1d-2 mass-scale)))
          ...
          (defparameter *terminus-mps*
            (loop for mp across mps
                  when (= x-max (magicl:tref
                                 ...
                                 0 0))
                    collect mp)))
        (let ((y-max (loop for mp across mps
                           maximize (magicl:tref
                                     (cl-mpm/particle:mp-position mp)
                                     1 0))))
          (defparameter *slice-mps*
            (loop for mp across mps
                  when (= y-max (magicl:tref
                                 (cl-mpm/particle:mp-position mp)
                                 1 0))
                    collect mp)))
        )
                       ...
                       (time (loop for i from 1 to substeps
                                   ...
                                   do
                                      (progn
                                        ...
                                        ;; (push
                                        ;; ;; *target-displacement*
                                        ;;  (* -1d0 cl-mpm/penalty::*debug-force*)
                                        ;;       *data-load*)
                                        ;; (push
                                        ;;  ;; *t*
                                        ;;  (get-disp *terminus-mps*)
           ...
           (let ((elements (expt 2 i))
             ...
             (let ((mesh-size (/ 5d0 elements))
               ...
               (defparameter *sim* (setup-test-column (list 10 mesh-size)
                                                      (list bar-length mesh-size) (list 0 0) (/ 1 mesh-size) mps-per-cell))
...
(defun plot-damage ()
    (let* ((x (loop for mp in *slice-mps*
      ...
      (with-open-file (stream (merge-pathnames "output/damage_dist.csv") :direction :output :if-exists :supersede)
        (format stream "x,d~%")
        (loop for ix in x
              for id in d
              do (format stream "~f, ~f ~%" ix id)))
      ))
  ...
  (with-accessors ((mps cl-mpm:sim-mps))
    ...
    (let* ()
      ...
      (vgplot:plot
       ...
       (mapcar (lambda (x) (* 1d0 x)) *data-disp*) (mapcar (lambda (x) (/ x (let ((max-e (reduce #'max *energy-dissipation*))) (if (= 0 max-e) 1d0 max-e)))) *energy-dissipation*) "energy"
                                        ;(mapcar (lambda (x) (/ x 1d0)) *time*) *max-damage* "MPM"
       ;; (mapcar (lambda (x) (/ x 1d0)) *time*) *max-damage* "MPM"
       )
      ;; (vgplot:figure)
  ...
  (let ((iters 1000000))
    (time
     (lparallel::pdotimes (i iters)
       (let ((strain (cl-mpm/utils:voigt-zeros))
             (df (cl-mpm/utils:matrix-from-list (list 1d0 0d0 0d0
                                                      0d0 1d0 0d0
                                                      0d0 0d0 1d0))))
         (cl-mpm/ext::kirchoff-expt-step-lisp strain df)
         )))
