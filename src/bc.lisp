...
(defmethod apply-bc ((bc bc-surface) node mesh dt)
  ...
  (with-slots ((normal normal))
    ...
    (with-accessors ((node-vel cl-mpm/mesh:node-velocity)
      ...
      (let ((rel-vel (magicl::sum (magicl.simd::.*-simd normal node-vel)))
            (rel-force (magicl::sum (magicl.simd::.*-simd normal node-force)))
            (rel-acc (magicl::sum (magicl.simd::.*-simd normal node-acc)))
            ;; (volume-ratio (min 1d0 (max 0d0 (/ volume vt))))
            (volume-ratio 1d0)
            )
        ;; (when (< rel-vel 0)
          ;; (magicl.simd::.+-simd node-vel (magicl:scale normal (sqrt (abs rel-vel))) node-vel)
          ;; (magicl.simd::.+-simd node-force (magicl:scale normal (sqrt (abs rel-force))) node-force)
          ;; )
        ;; (when (< rel-force 0)
        ;;   (magicl.simd::.+-simd node-force (magicl:scale normal (sqrt (abs rel-force))) node-force))
          ;; (magicl.simd::.+-simd node-acc (magicl:scale normal (sqrt (abs rel-acc))) node-acc)
        ...
        (when (< rel-force 0)
          ...
          ;;                               ;(magicl.simd::.+-simd node-vel (magicl:scale normal (sqrt rel-vel)) node-vel)
          ;;   (setf (magicl:tref node-vel 1 0) 0d0)
...
(defmethod apply-bc ((bc bc-friction) node mesh dt)
  ...
  (with-slots ((normal normal)
    ...
    (with-accessors (
      ...
      (let ((rel-vel (magicl::sum (magicl.simd::.*-simd node-vel normal))))
        (when t;(< rel-vel 0)
          ;Note this is momentum
          (let* ((tang-p (magicl:.- node-vel (magicl:scale normal rel-vel)))
                 (h (cl-mpm/mesh:mesh-resolution mesh))
                 ;; (normal-force (magicl::sum (magicl.simd::.*-simd node-force normal)))
            ...
            (setf node-force (magicl:.- node-force friction-force))
            ;; (setf node-vel (magicl.simd::.+-simd node-vel (magicl:scale tang-p -0.5d0)))
            ))
...
(defmethod apply-bc ((bc bc-body-force) node mesh dt)
  ...
  (with-slots ((value value))
    ...
    ;;       (magicl.simd::.+-simd (cl-mpm/mesh:node-force node)
...
(defun make-outside-bc-var-list (mesh left right top bottom)
  ...
  (with-accessors ((mesh-count cl-mpm/mesh:mesh-count)
      ...
      mesh
    (destructuring-bind (xsize ysize) (mapcar (lambda (x) (- x 1 order)) mesh-count)
      (remove nil
              (loop for o from 0 to order
                    append
                    (append
