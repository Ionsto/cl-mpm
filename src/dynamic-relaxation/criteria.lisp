           ...
           (lambda (node)
             (with-accessors ((active cl-mpm/mesh::node-active)
               ...
               (if (and active
                   ...
                   (setf node-oobf
                         (if (> oobf 0d0)
                             (cl-mpm/fastmaths::mag-squared f-int)
  ...
  (cl-mpm::reduce-over-cells
   ...
   (lambda (c)
     (if (and (cl-mpm/mesh::cell-active c)
         ...
         (let* ((df (cl-mpm/mesh::cell-deformation-gradient c))
  ...
  (cl-mpm::reduce-over-cells
   ...
   (lambda (c)
     (if (and (cl-mpm/mesh::cell-active c)
              (not (cl-mpm/mesh::cell-partial c)))
         (let* ((df (cl-mpm/mesh::cell-deformation-gradient c))
                (lens (magicl:@ df (cl-mpm/utils:vector-from-list (list 1d0 1d0 1d0)))))
           (let ((lens-3d (list (cl-mpm/utils:varef lens 0)
                                (cl-mpm/utils:varef lens 1)
                                (cl-mpm/utils:varef lens 2))))
             (/ (reduce #'max lens-3d)
                (reduce #'min lens-3d))))
