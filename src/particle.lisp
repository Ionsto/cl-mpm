...
(defmethod constitutive-model ((mp particle-elastic-damage) strain dt)
  ...
  (with-slots ((E E)
    ...
    (let ((elastic-increment (cl-mpm/constitutive::linear-elastic-mat strain-rate de)))
      (magicl:.+
       stress-undamaged
      ...
      ;; (setf stress (magicl:scale stress-undamaged (max 0d0 (expt (- 1d0 damage) 2d0))))
      (magicl::scale! elastic-increment (max 0d0 (expt (- 1d0 damage) 2d0)))
      (magicl:.+
       stress
       (objectify-stress-logspin
        elastic-increment
    ...
    ;;  stress
    ;;  (objectify-stress-logspin
    ;;   (magicl:scale! (cl-mpm/constitutive::linear-elastic-mat strain-rate de) (max 1d-3 (- 1d0 damage)))
    ;;   stress
    ;;   def
    ;;   vorticity
    ;;   D
    ...
    ;;        (pressure-matrix (magicl:eye 2 :value p))
    ;;        (dev-stress (magicl:.- stress-matrix pressure-matrix)))
    ;;   (setf stress (matrix-to-voight
    ;;                 (magicl:.+ pressure-matrix
    ;;                                   (magicl:scale! dev-stress (max 1d-3 (- 1d0 damage)))))))
    ;; (when (> damage 0.0d0)
    ;;   (setf (magicl:tref stress 2 0) (* (magicl:tref stress 2 0)
    ;;                                     (max 1d-5 (- 1d0 damage)))))
    ;; (when t;(> damage 0.0d0)
    ...
    ;;                      (when (> esii 0d0)
    ;;                        (setf (nth i l)
    ;;                              (* (nth i l) (expt (- 1d0 damage) 2d0))
    ;;                              ;; (+
    ;;                              ;;  (* esii (expt (- 1d0 damage) 2d0))
    ;;                              ;;  pressure-ke)
    ;;                              ))
...
(defmethod constitutive-model ((mp particle-viscoplastic-damage) strain dt)
  ...
  (with-slots ((E E)
    ...
    (let* ((viscosity (cl-mpm/constitutive::glen-viscosity-strain eng-strain-rate visc-factor visc-power))
      ...
      ;;        (objectify-stress-logspin
      ;;         (if (> viscosity 0d0)
      ;;             (cl-mpm/constitutive::maxwell-damage strain-rate stress E nu de viscosity dt damage strain)
      ;;             (cl-mpm/constitutive::linear-elastic-mat strain-rate de))
      ;;         stress
      ;;         def
      ;;         vorticity
      ;;         D)))
      ;; (setf stress (magicl:scale stress-u (max 1d-4 (- 1d0 damage))))
      ;; (let* ((stress-matrix (voight-to-matrix stress))
      ;;        (p (/ (magicl:trace stress-matrix) 2d0))
      ;;        (pressure-matrix (magicl:eye 2 :value p))
      ;;        (dev-stress (magicl:.- stress-matrix pressure-matrix)))
      ;;   (setf stress (matrix-to-voight
      ;;                 (magicl:.+ pressure-matrix
      ;;                                   (magicl:scale! dev-stress (max 1d-8 (- 1d0 damage)))))))
      ;; (when (> damage 0.0d0)
      ;;   (setf (magicl:tref stress 2 0) (* (magicl:tref stress 2 0)
      ;;                                     (max 1d-5 (- 1d0 damage)))))
              ...
              (loop for i from 0 to 1
                    do (let* ((sii (nth i l))
                         ...
                         (when (< esii 0d0)
                           ...
                           (setf (nth i l) (* sii (max 1d-4 (- 1d0 damage))))
                           )
                         ;; (setf (nth i l) (* sii (max 0d0 (- 1d0 damage))))
                         ))
