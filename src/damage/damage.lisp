...
(defmethod cl-mpm/particle::compute-mp-energy-release ((mp cl-mpm/particle::particle-damage))
  (with-accessors ((volume cl-mpm/particle::mp-volume)
      ...
      mp
    (declare (double-float volume damage-inc))
      ...
      (when non-local-damage
        ...
        (unless (= delocal-counter-max -1)
          (when (<= delocal-counter 0)
            ...
            (update-delocalisation-list sim)
...
(defun weight-func-grads (dist-squared length)
  (declare (double-float dist-squared length))
  (if (< dist-squared (* length length))
      (let ((dist (sqrt dist-squared)))
        (declare (double-float dist))
        (the double-float
             (* 4d0
                (-
                 (/ (expt dist 3)
    ...
    (defun weight-func-mps-gradient-trapezium (mesh mp-a mp-b pos-a pos-b length)
      ...
      (let* ((da (cl-mpm/particle::mp-av-damage mp-a))
        ...
        (flet ((deg (d) (max 1d-9 (the double-float (sqrt (max 0d0 (min 1d0 (- 1d0 d))))))))
          (let* ((d0 da)
                 (d1 (min 1d0 (max 0d0 (+ da (cl-mpm/fastmaths::dot (cl-mpm/particle::mp-av-damage-gradient mp-a) delta)))))
...
(defun patch-in-bounds-2d (mesh pos bound)
  (destructuring-bind (x y z) pos
    ...
    (and
     (cl-mpm/mesh::in-bounds-values mesh (+ x bound) (+ y bound) z)
     (cl-mpm/mesh::in-bounds-values mesh (- x bound) (- y bound) z)
...
(declaim (inline iterate-over-damage-bounds))
...
(defun iterate-over-neighbour-mps (mesh mp length func)
  ...
  (let (;; (pos (cl-mpm/particle::mp-position mp))
      ...
      (loop for mp-other across local-list
            do (progn
                 (funcall func
                          ...
                          ;;            (cl-mpm/fastmaths::diff-norm
                          ;;             pos
                          ;;             (cl-mpm/particle::mp-position (the cl-mpm::particle mp-other))))))
                          ))))
    ;; (iterate-over-damage-bounds
    ;;  mesh
    ;;  pos
    ...
    ;;                 (funcall func mp-other (the double-float (sqrt distance))))))))
    )
  (values))

(declaim (inline iterate-over-point-neighbour-mps))
             ...
             do
                (with-accessors ((d cl-mpm/particle::mp-damage)
                  ...
                  (let ((distance (cl-mpm/fastmaths::diff-norm
                    ...
                    (when (< distance len-squared)
                      (funcall func mesh mp-other (the double-float (sqrt distance)))))))
       (values))))
  (values))
...
(defun calculate-delocalised-damage (mesh mp length length-localisation)
  (let ((damage-inc 0d0)
    ...
    (when (> ;; mass-total
      ...
      (setf (aref vals 0)
            (if +damage-average-energy+
                (sqrt (/ (aref vals 0) (aref vals 1)))
                (/ (aref vals 0) (aref vals 1))))
      ;; (setf (aref vals 0) (/ (aref vals 0) (aref vals 1)))
...
(defmethod update-localisation-lengths ((sim cl-mpm/damage::mpm-sim-damage))
  (with-accessors ((mps cl-mpm::sim-mps)
                   (mesh cl-mpm:sim-mesh)
                   (stress-based sim-enable-stress-based-length)
                   )
      sim
    (cl-mpm::iterate-over-mps
     mps
...
(cl-mpm/utils::with-matrix-pool 
    (defmethod delocalise-damage-stress ((sim cl-mpm/damage::mpm-sim-damage))
      (declare (optimize (speed 3)))
      (with-accessors ((mps cl-mpm::sim-mps)
                       (mesh cl-mpm:sim-mesh))
          sim
         ...
         (lambda (mp)
           (when (typep mp 'cl-mpm/particle:particle-damage)
             (let ((stress (cl-mpm/particle::mp-undamaged-stress mp))
               ...
               (multiple-value-bind (l v) (cl-mpm/utils::eig (cl-mpm/utils::voight-to-matrix stress (grab-new-matrix)))
         ...
         (lambda (mp)
           (when (typep mp 'cl-mpm/particle:particle-damage)
             (with-accessors ((damage-ybar cl-mpm/particle::mp-damage-ybar)
                              ...
                              (local-length-t cl-mpm/particle::mp-local-length))
                 mp
               (setf damage-ybar (calculate-delocalised-damage-stress mesh mp local-length-t)))))))
      (values)))
    ...
    (when (> (length mps) 0)
      (loop for mp across mps
            when (and
                  ...
                  (funcall func mp))
              do (progn
                   (setf (cl-mpm/damage::sim-dirty-local-list sim) t)
                   (local-list-remove-particle mesh mp)))
      (call-next-method))))
...
(defmethod cl-mpm::remove-mps-func :after ((sim mpm-sim-damage) func)
  (with-accessors ((mesh cl-mpm::sim-mesh)
                   (mps cl-mpm::sim-mps))
      sim
    (when (cl-mpm/damage::sim-dirty-local-list sim)
      (setf (cl-mpm/damage::sim-dirty-local-list sim) nil)
      (setup-mp-local-list sim))))
...
(defmethod cl-mpm::add-mps-finalise ((sim mpm-sim-damage))
  ;; (format t "Re-setup mps~%")
  (when (cl-mpm/damage::sim-dirty-local-list sim)
    (setf (cl-mpm/damage::sim-dirty-local-list sim) nil)
    (setup-mp-local-list sim)))
...
;;                                  (mass cl-mpm/particle::mp-mass))
;;                     mp
;;                   (declare (double-float mass damage damage-prev))
;;                   (expt (* mass (- damage damage-prev)) 2))
;;                 0d0))
;;             #'+))
;;          (delta-incs
...
(defun get-nonlocal-interactions (sim mp)
  (cl-mpm::iterate-over-mps
   (cl-mpm:sim-mps sim)
   (lambda (mp)
     (setf (cl-mpm/particle::mp-debug-j mp) 0d0)))
  (cond
    ((cl-mpm/damage::sim-enable-ekl sim)
     (get-nonlocal-interactions-ekl sim mp))
    ((cl-mpm/damage::sim-enable-stress-based-length sim)
     (get-nonlocal-interactions-stress-based sim mp))
    (t
     (when (cl-mpm/damage::sim-enable-length-localisation sim)
       (update-localisation-lengths sim))
     (get-nonlocal-interactions-standard sim mp))))

(defun get-nonlocal-interactions-standard (sim mp)
  (g2p-damage sim)
  (with-accessors ((mesh cl-mpm:sim-mesh)
                   (mps cl-mpm:sim-mps))
      sim
    (let ((data-positions (list))
      ...
      (when (> mass-total 0d0)
        (cl-mpm:iterate-over-mps
         mps
         (lambda (mp)
           (setf (cl-mpm/particle::mp-debug-j mp) (/ (cl-mpm/particle::mp-debug-j mp) mass-total))))
        (setf damage-inc (/ damage-inc mass-total))
...
(defun get-nonlocal-interactions-ekl (sim mp)
  (g2p-damage sim)
  (with-accessors ((mesh cl-mpm:sim-mesh)
    ...
    (let ((data-positions (list))
          ...
          (mass-total 0d0)
          (length (* 1d0 (cl-mpm/particle::mp-local-length mp))))
      (declare (double-float mass-total))
      (iterate-over-neighbour-mps
       mesh mp length
       (lambda (mp-other)
         (with-accessors ((d cl-mpm/particle::mp-damage)
           ...
           (let ((weight (weight-func-mps-damaged mesh mp mp-other length)))
             (declare (double-float weight m d mass-total))
             (incf mass-total (* weight m))
             (push (* weight m) data-weights)
             (setf (cl-mpm/particle::mp-debug-j mp-other) (* weight m))
             ))))
...
(defun get-nonlocal-interactions-stress-based (sim mp)
  (g2p-damage sim)
  (with-accessors ((mesh cl-mpm:sim-mesh)
                   (mps cl-mpm:sim-mps)) sim
    (let ((data-positions (list))
