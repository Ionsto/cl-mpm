...
(defmethod apply-buoyancy ((sim cl-mpm::mpm-sim) func-stress func-div clip-function datum)
      ...
      (cl-mpm:iterate-over-nodes
       ...
       (lambda (node)
         (when (cl-mpm/mesh::node-active node)
           (with-accessors ((boundary-scalar cl-mpm/mesh::node-boundary-scalar)
                            (boundary-vec cl-mpm/mesh::node-boundary-vec))
               node
             (declare (double-float boundary-scalar))
...
(defmethod apply-force-cells-3d ((sim cl-mpm::mpm-sim) func-stress func-div clip-func)
     ...
     (lambda (cell)
       ...
       (with-accessors (;(pos cl-mpm/mesh::cell-centroid)
         ...
         (let* (;; (pos (cl-mpm/fastmaths:fast-.+ pos disp))
                ...
                (cell-stress (calculate-val-cell cell func-stress))
                ...
                (let (;(grads (cl-mpm::gradient-push-forwards grads df))
                  ...
                  (when (and node-active
                    ...
                    (sb-thread:with-mutex (node-lock)
                      ...
                      ;;  (cl-mpm/mesh::node-boundary-vec node))
                      ;; (cl-mpm/fastmaths:fast-.- node-buoyancy-force (cl-mpm/fastmaths::fast-.+ f-stress f-div) node-buoyancy-force)
  ...
  (with-accessors ((mesh cl-mpm:sim-mesh)
    ...
    (with-accessors ((h cl-mpm/mesh:mesh-resolution))
      ...
      (cl-mpm::iterate-over-nodes
       ...
       (lambda (n)
         (when (cl-mpm/mesh:node-active n)
           (cl-mpm/fastmaths::fast-zero (cl-mpm/mesh::node-boundary-vec n))
           (setf (cl-mpm/mesh::node-boundary-scalar n) 0d0))))
      (apply-scalar-mps-3d
       mesh
       mps
       (lambda (mp)
         (calculate-val-scalar-mp-gimp mesh mp func-scalar))
       clip-function
       :damage-volume damage-volume)
      (cl-mpm:iterate-over-nodes
       mesh
   ...
   (lambda (cell)
     (when t;(loop for n in (cl-mpm/mesh::cell-nodes cell) thereis (cl-mpm/mesh::node-boundary-node n))
       (let ((nodal-volume 0d0))
         (when t;(funcall clip-func (cl-mpm/mesh::cell-centroid cell))
           ;;Iterate over a cells nodes
           (cl-mpm::cell-iterate-over-neighbours
            mesh cell
...
(defun apply-scalar-mps-3d (mesh mps func-scalar clip-func &key (damage-volume nil))
  "Update force on nodes, with virtual stress field from mps"
  (declare (function func-scalar clip-func))
  (cl-mpm::iterate-over-mps
   mps
   (lambda (mp)
     (when t;(funcall clip)
       (with-accessors ((volume cl-mpm/particle::mp-volume)
                        ...
                        (damage cl-mpm/particle::mp-damage))
           mp
         (declare (double-float volume damage))
         (let ()
           ;;Iterate over neighbour nodes
           (cl-mpm::iterate-over-neighbours
            mesh mp
...
(defmethod cl-mpm/bc::apply-sim-bc (sim (bc bc-buoyancy-body) dt)
  ...
  (with-accessors ((datum bc-buoyancy-datum)
    ...
    (with-accessors ((mesh cl-mpm:sim-mesh)
      ...
      (apply-buoyancy-body
       mesh mps
       (lambda (mp) (calculate-val-mp
                     mp
                     (lambda (pos)
                       (cl-mpm/utils:vector-from-list (list 0d0 (* -1d0 rho (cl-mpm:sim-gravity sim)) 0d0))
...
(declaim (notinline apply-buoyancy-body))
(defun apply-buoyancy-body (mesh mps func-div clip-func datum)
  "Update force on nodes, with virtual stress field from mps"
               ...
               (with-accessors ((node-force cl-mpm/mesh::node-force)
                 ...
                 (when (and node-active
                   ...
                   (sb-thread:with-mutex (node-lock)
                     ...
                     (let* ((dist (max 0d0 (- datum (varef pos 1))))
                            (lp (/ (varef (cl-mpm/particle::mp-domain-size mp) 1) 2))
                            (factor (min 1d0 (abs (/ dist lp))))
                            (buoyancy-force (cl-mpm/fastmaths::fast-scale!
                                             (funcall func-div mp)
                                             (* svp volume factor))))
...
;; (defun compute-stiffness-mps-3d (mesh mps stiffness-func clip-func)
;;   "Update force on nodes, with virtual stress field from mps"
