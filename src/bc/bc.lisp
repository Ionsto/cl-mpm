...
(defmethod apply-bc ((bc bc-surface) node mesh dt)
  ...
  (with-slots ((normal normal))
    ...
    (with-accessors ((node-vel cl-mpm/mesh:node-velocity)
      ...
      (let ((rel-vel (magicl::sum (cl-mpm/fastmath::fast-.* normal node-vel)))
            (rel-force (magicl::sum (cl-mpm/fastmath::fast-.* normal node-force)))
            (rel-acc (magicl::sum (cl-mpm/fastmath::fast-.* normal node-acc)))
            ;; (volume-ratio (min 1d0 (max 0d0 (/ volume vt))))
            (volume-ratio 1d0)
            )
        ;; (when (< rel-vel 0)
          ;; (cl-mpm/fastmath::fast-.+ node-vel (magicl:scale normal (sqrt (abs rel-vel))) node-vel)
          ;; (cl-mpm/fastmath::fast-.+ node-force (magicl:scale normal (sqrt (abs rel-force))) node-force)
          ;; )
        ;; (when (< rel-force 0)
        ;;   (cl-mpm/fastmath::fast-.+ node-force (magicl:scale normal (sqrt (abs rel-force))) node-force))
          ;; (cl-mpm/fastmath::fast-.+ node-acc (magicl:scale normal (sqrt (abs rel-acc))) node-acc)
        ...
        (when (< rel-force 0)
          ...
          ;;                               ;(cl-mpm/fastmath::fast-.+ node-vel (magicl:scale normal (sqrt rel-vel)) node-vel)
          ;;   (setf (magicl:tref node-vel 1 0) 0d0)
...
(defmethod apply-bc ((bc bc-friction) node mesh dt)
  ...
  (with-slots ((normal normal)
    ...
    (with-accessors (
      ...
      (let ((rel-vel (magicl::sum (cl-mpm/fastmath::fast-.* node-vel normal))))
        (when t;(< rel-vel 0)
          ;Note this is momentum
          (let* ((tang-p (magicl:.- node-vel (magicl:scale normal rel-vel)))
                 (h (cl-mpm/mesh:mesh-resolution mesh))
                 ;; (normal-force (magicl::sum (cl-mpm/fastmath::fast-.* node-force normal)))
            ...
            (setf node-force (magicl:.- node-force friction-force))
            ;; (setf node-vel (cl-mpm/fastmath::fast-.+ node-vel (magicl:scale tang-p -0.5d0)))
            ))
...
(defmethod apply-bc ((bc bc-body-force) node mesh dt)
  ...
  (with-slots ((value value))
    ...
    ;;       (cl-mpm/fastmath::fast-.+ (cl-mpm/mesh:node-force node)
