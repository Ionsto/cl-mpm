...
(defun p2g-tflip-mp (mesh mp)
  ...
  (with-accessors ((mp-vel  cl-mpm/particle:mp-velocity)
                   ...
                   (mp-pmod cl-mpm/particle::mp-p-modulus)
                   (mp-disp cl-mpm/particle::mp-displacement-increment)
                   (mp-vel-grad cl-mpm/particle::mp-velocity-gradient)
                   (mp-position cl-mpm/particle:mp-position)
                   (mp-damage cl-mpm/particle::mp-damage))
      mp
    (let ((mp-mass mp-mass)
          (mp-vel mp-vel)
          ...
          (mp-pmod mp-pmod)
          (mp-damage mp-damage)
          (dist-vec (cl-mpm/utils:vector-zeros))
          )
      (declare (type double-float mp-mass mp-volume))
      (iterate-over-neighbours
       mesh mp
...
(defun p2g (mesh mps update-type)
