...
;; (defstruct (gradients-arr (:constructor make-gradients (dx dy dz)
;;                                         (setf (aref data 0) dx
;;                                               (aref data 1) dx
;;                                               (aref data 2) dx
;;                                               )
;;                                         ))
;;   (data (make-array 3 :element-type 'double-float) :type (simple-array double-float (3))))
                                     ...
                                     (let* ((dist (mapcar #'- pos (cl-mpm/mesh:index-to-position mesh id)))
                                       ...
                                       (when (< 0d0 weight)
                                         (funcall func node weight grads
                                                  0d0 (cl-mpm/utils::make-gradients 0d0 0d0 0d0))))))))))))
                          ...
                          (when (cl-mpm/mesh:in-bounds mesh id)
                            (let* ((dist (mapcar #'- pos (cl-mpm/mesh:index-to-position mesh id)))
                                   ...
                                   (grads (mapcar (lambda (d w) (* (the double-float (cl-mpm/shape-function::shape-linear-dsvp d h))
                                                                   (the double-float w)))
                                                  dist (nreverse weights)))
                                   (grads (cl-mpm/utils::make-gradients
                                           (nth 0 grads)
                                           (nth 1 grads)
                                           (nth 2 grads))))
                         ...
                         do
                            (when (cl-mpm/mesh::in-bounds-1d mesh (+ iy dy) 1)
                              (let ((dist-x (- pos-x (* h (float (+ ix dx) 0d0))))
                                ...
                                (declare (double-float dist-x dist-y))
                                (let* ((node (cl-mpm/mesh::get-node-values mesh (+ ix dx) (+ iy dy) 0))
                                  ...
                                  (declare (double-float weight-x weight-y grad-x grad-y weight))
                                  (when (< 0d0 weight)
                                      (funcall func node weight (cl-mpm/utils::make-gradients grad-x grad-y 0d0)))))))))))))
                ...
                (multiple-value-bind (cx cy) (sb-simd-avx:f64.2-values ca)
                  (multiple-value-bind (dxf dyf) (sb-simd-avx:f64.2-values dfa)
                    (multiple-value-bind (dxc dyc) (sb-simd-avx:f64.2-values dca)
                      ...
                      (let ((ix (the fixnum (truncate ix)))
                            (iy (the fixnum (truncate iy))))
                        (loop for dx fixnum from (the fixnum (truncate dxf)) to (the fixnum (truncate dxc))
                              do
                                 (when (cl-mpm/mesh::in-bounds-1d mesh (+ ix dx) 0)
                                   (loop for dy fixnum from (the fixnum (truncate dyf)) to (the fixnum (truncate dyc))
                                         do
                                            (when (cl-mpm/mesh::in-bounds-1d mesh (+ iy dy) 1)
                                                    ...
                                                    (let* ((weightsx (the double-float (cl-mpm/shape-function::shape-gimp distx (* 0.5d0 dox) h)))
                                                      ...
                                                      (when (< 0d0 weight)
                                                        (let* ((node (cl-mpm/mesh::get-node-values mesh (+ ix dx) (+ iy dy) 0))
                                                               ...
                                                               (dx (cl-mpm/shape-function::shape-gimp-dsvp distx (* 0.5d0 dox) h))
                                                               (dy (cl-mpm/shape-function::shape-gimp-dsvp disty (* 0.5d0 doy) h))
                                                               (gradx (* dx (the double-float weightsy)))
                                                               (grady (* dy (the double-float weightsx)))
                                                               (grad-fx (* weights-fbar-y dx))
                                                               (grad-fy (* weights-fbar-x dy)))
                                                          ...
                                                          (funcall func
                                                                   ...
                                                                   weight-fbar
                                                                   (cl-mpm/utils::make-gradients grad-fx grad-fy 0d0)))))))))))))))))))))))
                             ...
                             (let* ((node (cl-mpm/mesh:get-node mesh id))
                                    ...
                                    (grads-fbar
                                      (cl-mpm/utils::make-gradients
                                            ...
                                            0d0)))
                               (funcall func node
                                        weight (cl-mpm/utils::make-gradients gradx 0d0 0d0)
...
(declaim ;(inline iterate-over-neighbours-shape-gimp)
         (ftype (function (cl-mpm/mesh::mesh cl-mpm/particle:particle function) (values))
                iterate-over-neighbours-shape-gimp-3d))
(defun iterate-over-neighbours-shape-gimp-3d-lisp (mesh mp func)
  "Iterate over gimp neighbours in 3D, unrolled for speed"
  (declare (type cl-mpm/mesh::mesh mesh)
           (cl-mpm/particle:particle mp)
           (function func)
                                    ...
                                    (when (< 0d0 weight)
                                      (let* ((node (cl-mpm/mesh:get-node mesh id))
                                             ...
                                             (gradz (* (cl-mpm/shape-function::shape-gimp-dsvp distz doz h)
                                                       weightsx weightsy))
                                             (grads-fbar
                                               (cl-mpm/utils::make-gradients (* weights-fbar-y weights-fbar-z
                                                        (cl-mpm/shape-function::shape-gimp-dsvp distx dox h))
                                        ...
                                        (funcall func node
                                                 weight
                                                 (cl-mpm/utils::make-gradients gradx grady gradz)
                                                 weight-fbar grads-fbar)))))))))))))))
                                           ...
                                           (let* ((dista
                                             ...
                                             (when (< 0d0 weight)
                                               (let* ((node (cl-mpm/mesh:get-node mesh id))
                                                 ...
                                                 (declare (double-float gradx grady gradz))
                                                 (funcall func node
                                                          weight (cl-mpm/utils::make-gradients gradx grady gradz)
                                                          weight-fbar grads-fbar)))))))))))))))
...
(declaim (ftype (function (double-float fixnum fixnum) double-float)
                weights-local-pos))
(defun weights-local-pos (xi nd dxi)
  (let ((dxi (the fixnum (- (* dxi 2) 1))))
    (ecase nd
      (1 (* 0.5d0 (+ 1d0 (* dxi xi))))
      (2 (* 0.5d0 (+ 1d0 (* dxi xi))))
      (3 (* 0.5d0 (+ 1d0 (* dxi xi)))))))

(declaim (ftype (function (double-float fixnum fixnum double-float) double-float)
                grads-local-pos))
(defun grads-local-pos (x nd dx h)
  (let ((dx (the fixnum (- (* dx 2) 1))))
    (if (< (abs x) 1d-15)
        0d0
        (/
         (ecase nd
...
(defun iterate-over-cell-shape-local-1d (mesh cell local-position func)
  ...
  (declare (cl-mpm/mesh::mesh mesh)
           ...
           (magicl::matrix/double-float local-position)
           (function func))
  (progn
    (let* ((h (cl-mpm/mesh:mesh-resolution mesh))
           (nd 1)
                       ...
                       do
                          (let* ()
                            (let ((pvx (varef pos-vec 0))
                                  ...
                                  (cpy (varef cell-pos 1)))
                              (declare (double-float pvx pvy cpx cpy))
                              (let* ((node (aref cell-nodes i))
                                     (dist-x (* 2d0 (/ (- pvx cpx) h)))
                                     (dist-y (* 2d0 (/ (- pvy cpy) h)))
                                     (weight-x (weights-local-pos dist-x nd dx))
                                     (weight-y (weights-local-pos dist-y nd dy)))
                                (declare (double-float weight-x weight-y))
                     ...
                     do (loop for dz from 0 to 1
                              do (let* ()
                                   (let* ((node (aref cell-nodes i))
                                     ...
                                     (declare (double-float weight-x weight-y weight-z))
