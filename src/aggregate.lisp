...
(defun aggregate-vec (sim vec d &optional (res nil))
  ...
  ;;  vec)
  ;; (magicl:@
  ;;  (cl-mpm/utils:transpose (cl-mpm/aggregate::sim-global-e sim))
  ;;  vec)
  )
...
(defun iterate-over-cell-patch-1d (mesh position ring-size func)
  ...
  (let ((index (cl-mpm/mesh::position-to-index-cell mesh position)))
    (destructuring-bind (ix iy iz) index
      ...
      (loop for dx from (- (+ ring-size 1)) to ring-size
            do (when (cl-mpm/mesh::in-bounds-cell-1d mesh (+ ix dx) 0)
                 (let ((cell-index (mapcar #'+ index (list dx 0 0))))
...
(defun iterate-over-cell-patch (mesh position ring-size func)
  ...
  (case (cl-mpm/mesh::mesh-nd mesh)
    (1 (iterate-over-cell-patch-1d mesh position ring-size func))
    (2 (iterate-over-cell-patch-2d mesh position ring-size func))
    (3 (iterate-over-cell-patch-3d mesh position ring-size func)))
...
;; (defun get-best-cell (mesh position
;;                       &key (exclude nil)
    ...
    (flet ((check-cell (cell)
             (with-accessors ((mp-count cl-mpm/mesh::cell-mp-count)
               ...
               (when (and
                      ...
                      (not (eq cell exclude))
                      (funcall filter cell))
                 (let ((vol-ratio (/ volume-t volume))
                       (dist-tr (cl-mpm/fastmaths::diff-mag pos centroid)))
                   (when (> dist-tr h)
                     (when (or
                            (not closest-elem)
                            ;; (and
...
(declaim (notinline linear-solve-with-bcs)
         (ftype (function (cl-mpm/aggregate::mpm-sim-aggregated
...
(cl-mpm/utils::with-arb-pool
  (defun linear-solve-with-bcs (sim ma v d &optional (target-vi nil))
    (let ((target-vi (if target-vi
                         ...
                         (cl-mpm/utils::arb-matrix (cl-mpm/utils:nrows v) 1))))
      (unless (= (magicl::matrix/double-float-ncols target-vi) 0)
        (let* ((et (cl-mpm/aggregate::sim-global-sparse-et sim))
               (e (cl-mpm/aggregate::sim-global-sparse-e sim))
               (sma (cl-mpm/aggregate::sim-global-sparse-ma sim))
