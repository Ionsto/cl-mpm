...
(eval-when (:compile-toplevel :load-toplevel :execute)
  ...
  (defparameter *git-hash* "NO-HASH")
  (defun read-current-git-hash ()
    (with-open-file (fs (asdf:system-relative-pathname :cl-mpm "./config/git-commit-version.txt") :direction :input)
      (defparameter *git-hash* (read fs)))
    (format t "Git commit ~A~%" cl-mpm::*git-hash*))
  (read-current-git-hash)
  (defun build-git-hash ()
    (format t "Git commit ~A~%" cl-mpm::*git-hash*)))
...
(declaim (inline integrate-vel-midpoint)
         (ftype (function (magicl::matrix/double-float
                           magicl::matrix/double-float
                           double-float double-float double-float double-float) (vaules)) integrate-vel-midpoint))
...
(defun update-particle-kirchoff (mesh mp dt)
  (with-accessors ((disp cl-mpm/particle::mp-displacement)
                   ...
                   (friction-force-prev cl-mpm/particle::mp-penalty-frictional-force-prev)
...
(declaim (ftype (function (cl-mpm/utils::gradients magicl::matrix/double-float) cl-mpm/utils::gradients) gradient-push-forwards-cached))
(defun gradient-push-forwards-cached (grads df-inv)
  ...
  (let ((dx (cl-mpm/utils::gradients-dx grads))
        ...
        (dz (cl-mpm/utils::gradients-dz grads)))
    (declare (double-float dx dy dz))
    (let ((x (+ (* dx (the double-float (mtref df-inv 0 0)))
                (* dy (the double-float (mtref df-inv 1 0)))
                (* dz (the double-float (mtref df-inv 2 0)))))
          (y
            (+ (* dx (the double-float (mtref df-inv 0 1)))
               (* dy (the double-float (mtref df-inv 1 1)))
               (* dz (the double-float (mtref df-inv 2 1)))))
          (z
            (+ (* dx (the double-float (mtref df-inv 0 2)))
               (* dy (the double-float (mtref df-inv 1 2)))
               (* dz (the double-float (mtref df-inv 2 2))))))
  ...
  (let ((mesh (cl-mpm:sim-mesh sim)))
    (let* ((node-count (array-total-size (cl-mpm/mesh::mesh-nodes mesh)))
      ...
      (cl-mpm:iterate-over-mps
       (cl-mpm:sim-mps sim)
       (lambda (mp)
         (let ((index (cl-mpm/mesh::position-to-index-round mesh (cl-mpm/particle:mp-position mp))))
           (if (cl-mpm/mesh::in-bounds mesh index)
